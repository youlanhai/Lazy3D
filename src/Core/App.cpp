...
LRESULT CALLBACK app_defWndProc_(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (g_pApp != nullptr && Lazy::processWindowMsg(g_pApp, hWnd, msg, wParam, lParam))
        return 0;
...
{
    if (!Lazy::rcDevice()->create(m_hWnd, m_hInstance, m_bFullScreen))
        return false;

    m_pd3dDevice = Lazy::rcDevice()->getDevice();
    m_pd3dDevice->AddRef();
...
bool CApp::init(void)
{
    ...
    Lazy::FontMgr::instance()->registerFontFile(L"C:/Windows/Fonts/simhei.ttf", L"def");
    ...
    addUpdater(Lazy::CallbackMgr::instance());
...
bool CApp::onEvent(const Lazy::SEvent & event)
{
    if (event.eventType == Lazy::EET_KEY_EVENT)
    {
        if (!event.keyEvent.down && event.keyEvent.key == Lazy::KEY_ESCAPE)
        {
