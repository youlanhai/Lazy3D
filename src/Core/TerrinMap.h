...
//////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        bool createMap(const tstring & mapName, int rows, int cols);

        bool createMap(float widht, float height, const tstring & mapName);

        ///导入高度图
        bool importHeightMap(const tstring & config);

        ///导出高度图
        bool exportHeightMap(const tstring & config);

        ///创建一个新的地形块
        bool createChunk(float x, float y);

        ///导入一个地形块
        bool importChunk(const tstring & file);
