...
namespace Physics
{
    ...
    public:
        ...
        MapRayCollider(const Physics::Vector3 & o, const Physics::Vector3 & d)
            : v3Origin(o)
            , v3Dir(d)
            , distance(Physics::MAX_FLOAT)
        {
            origin.set(o.x, o.z);
            dir.set(d.x, d.z);
        }
    ...
    public:
        ...
        MapRayAndObjCollider(const Physics::Vector3 & o, const Physics::Vector3 & d)
            : v3Origin(o)
            , v3Dir(d)
        {
            origin.set(o.x, o.z);
...
bool TerrainMap::loadMap(const Lazy::tstring & mapName)
{
    ...
    Lazy::LZDataPtr ptr = Lazy::openSection(mapName);
    if (!ptr)
    {
...
void TerrainMap::saveMap(const Lazy::tstring &)
{
    doDataConvert();
}
...
void TerrainMap::initChunks(void)
{
    ...
    Physics::qtree::Config = { 14, 1, 1000.0f, true };
    Physics::RectArray rects;
...
#if 0
    //将四叉树数据保存到文件中
    m_quadTree.save(_T("quadTree.lzd"), Lazy::LZType::lzd);
...
void TerrainMap::processExternalItem(TerrainItemPtr item)
{
    ...
    Physics::MapRectCollider collider(false);
...
void TerrainMap::render(IDirect3DDevice9* pDevice)
{
    ...
    dx::Texture *pTex = Lazy::TextureMgr::instance()->getTexture(m_textureName);
...
ChunkPtr TerrainMap::currentNode()
{
    if (m_pSource)
    {
        const Math::Vector3 & pos = m_pSource->getPos();
...
bool TerrainMap::intersect(Physics::Vector3 & position)
{
    cPick *pick = getPick();
    if (NULL == pick) return false;

    Physics::MapRayCollider collider(pick->rayPos(), pick->rayDir());
...
void TerrainMap::handeMouseEvent(UINT msg, WPARAM, LPARAM )
{
    ...
    if (!cPick::instance()->isTerrainObjEnable())
    {
        return;
    }
...
bool TerrainMap::createMap(float width, float height, const Lazy::tstring & mapName)
{
...
bool TerrainMap::createMap(const Lazy::tstring & mapName, int rows, int cols)
{
    if (rows <= 0) rows = 1;
    if (cols <= 0) cols = 1;
